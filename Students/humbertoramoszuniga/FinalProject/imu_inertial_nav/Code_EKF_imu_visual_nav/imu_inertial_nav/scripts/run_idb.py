...
##The data for the report was generated with data_and_camera_2.idb.
...
distorsion_coeffs = np.array([-0.213922, 0.088003, 0.000180, -0.000312, 0.000000], dtype=np.float32)
# Number of features.
m = 16
# Testing
...
bias_g_est = w_bias_est.reshape(3, 1)
...
# bias_a_est = np.array([[  2.04851808e-03], [  8.12233271e-04],       [ -6.73392292e-05]])

# bias_g_est = np.array([0, 0, 0]).reshape(3, 1)
# bias_g_est = np.array([0, 0, 0],dtype=np.float32).reshape(3, 1)
...
bias_a_est_history = np.array([0., 0, 0]).reshape(3, 1)
...
# Testing P
P = np.diag(np.concatenate([[0.0002] * 3, [0.0001] * 3, [0.01] * 3, [0.00001] * 3, [0.02] * 3]))
...
# Q testing
Q = np.diag(np.concatenate([[0.3] * 3, [0.015] * 3, [0.0001] * 3, [0.00002] * 3])) ** 2
# Q = np.diag(np.concatenate([[0.02] * 3, [0.015] * 3, [0.0000000001] * 3, [0.00002] * 3])) ** 2
# Testing
...
def omegahere(self):
    ...
    """
    omega1 = np.zeros((4, 4))
    omega1[:3, :3] = -skew_symmetric(self)
    omega1[:3, 3:4] = self
    omega1[3:4, 0:3] = -self.T
    return omega1
...
while clock():
    ...
    if imu.poll():
        ...
        if num_imu_measure > 0:
            ...
            w_hat = AngularRate(np.array(w) - bias_g_est)
            ...
            bias_a_est_history = np.append(bias_a_est_history, bias_a_est, axis=1)
    ...
    if feature_detector.poll():
        if num_imu_measure > 1:
            ...
            v_W_hat = v_W_hat + (np.dot(q_W_to_I.asRM(), s_hat) + g_W) * DTi
            # print "Velocity",v_W_hat
            # print "\n"
            ...
            # Compute Updated bounds for errors from the covariance matrix
            P_position_diag = np.diagonal(P[3:6, 3:6]).reshape(3, 1)
            ncol = sigmas_position_history.shape[1]
            sigmas_position_history[:, ncol - 1:] = np.sqrt(P_position_diag) * 3
            ...
            for f_id, feature in enumerate(feature_measurement.features[:m]):
                ...
                tt = np.array([[[feature.u, feature.v]]])
                undistort_y = cv2.undistortPoints(tt, cameraMatrix, distorsion_coeffs, P=cameraMatrix)
                undistort_y = undistort_y.flatten().reshape(2, 1)
                # print tt
                y_measurement[row_slice2, :] = undistort_y
                ...
                y_f_hat_i = np.array([[fx * (h_hat_i.x / h_hat_i.z) + cx], [fy * (h_hat_i.y / h_hat_i.z) + cy]])
                ...
                partial_y_measure_p_fC = (1. / h_hat_i.z) * np.array([[fx, 0, -fx * h_hat_i.x / h_hat_i.z],
                                                                      [0, fy, -fy * h_hat_i.y / h_hat_i.z]]).reshape(2,
                                                                                                                    3)
                # Blocks that cvom
                q_block = skew_symmetric(
                    q_W_to_I.asDCM().dot(np.array([p_fC_i.x, p_fC_i.y, p_fC_i.z]).reshape(3, 1) - p_IW_hat))
                pos_block = -q_W_to_I.asDCM()
                vel_block = np.zeros((3, 3))
                biasg_block = np.zeros((3, 3), dtype=np.float)
                biasa_block = np.zeros((3, 3), dtype=np.float)

                partial_p_FC_x_error = q_ic.asDCM().dot(
                    np.hstack((q_block, pos_block, vel_block, biasg_block, biasa_block)))

                H[row_slice2, :] = partial_y_measure_p_fC.dot(partial_p_FC_x_error)
            ...
            quat_error = Quat(q_vec)
            quat_error.normalize()
            #
            q_W_to_I = quat_error * q_W_to_I
            q_W_to_I.normalize()
            ...
            # if camera is not None and camera.poll():
            #     print "camera time",camera.latest().true_time
...
plotter_list.append(bias_g_est_plotter)

# # create and add data to plotter for acc bias
bias_a_est_plotter = MultiPlotter((3, 1), (6, 6), "Acc bias", dict(sharex=True))
bias_a_est_plotter.add_data("all", sim_time, bias_a_est_history.T, labels="Estimate")
bias_a_est_plotter.set_axis_titles(2, "Time", "")
bias_a_est_plotter.set_plot_titles("all", ["x", "y", "z"])
plotter_list.append(bias_a_est_plotter)
...
save_dir = "/home/humberto/Dropbox/PHD/662 Detection/Final project 662 Detection/fig"
#
save_args = {}
# for Latex, use
