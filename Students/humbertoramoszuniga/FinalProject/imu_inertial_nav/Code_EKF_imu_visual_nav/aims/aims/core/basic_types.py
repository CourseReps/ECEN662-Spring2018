...
class EstimatedState(StateContainer):
    """
    Structure containing time history of estimated states.
    """
    def __init__(self,length):
        self.time = NanArray(length)
        self.position = XYZArray(length)
...
class ThreeSigma(StateContainer):
    def __init__(self,length):
        self.time = NanArray(length)
        self.imu_state = NanArray((15,length))
